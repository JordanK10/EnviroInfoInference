...
def create_figure1a(dummy_data, save_path="figure1"):
    ...
    ci_lower = empirical_mean - 1.96 * empirical_std / np.sqrt(n_agents)
    ci_upper = empirical_mean + 1.96 * empirical_std / np.sqrt(n_agents)
    ...
    middle_incomes = log_incomes[middle_agents, :] if len(middle_agents) > 0 else np.zeros((1, n_timesteps + 1))
    ...
    middle_mean = np.mean(middle_incomes, axis=0) if len(middle_agents) > 0 else np.zeros(n_timesteps + 1)
    ...
    if len(bottom_agents) > 1:
        ...
        bottom_ci = 1.96 * bottom_std / np.sqrt(len(bottom_agents))
    ...
    if len(middle_agents) > 1:
        ...
        middle_ci = 1.96 * middle_std / np.sqrt(len(middle_agents))
    ...
    if len(top_agents) > 1:
        ...
        top_ci = 1.96 * top_std / np.sqrt(len(top_agents))
...
def main():
    ...
    # Path to dummy data
    data_path = "figure1/dummy_data_kelly_betting_static_x.pkl"
