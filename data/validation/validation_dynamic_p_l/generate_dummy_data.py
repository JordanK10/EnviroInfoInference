...
def generate_dummy_data(n_agents=1000, n_timesteps=30, 
    ...
    agent_beliefs = np.random.uniform(0.52, 0.55, n_agents)
    # agent_beliefs = [.6 for a in range(int(n_agents))] 
    
    # Initialize agent resources
    agent_resources = np.exp(np.random.normal(11, .5, n_agents))
    ...
    l = np.random.uniform(1.8,2.2)
    # l = 2
    
    # p: depends on l (negative correlation)
    if l < 2.2:
        p = np.random.uniform(0.65, 0.7)  # l<2.0: p uniform 0.6-0.8
    else:  # l >= 2.0
        p = np.random.uniform(0.6, 0.6)  # l>=2.0: p uniform 0.4-0.5
...
def main():
    ...
    USE_DYNAMIC_X = False  # Set to False for static beliefs, True for dynamic beliefs
