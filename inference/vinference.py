...
def bayesian_p_estimation(growth_rates_data, population_weights=None, 
                         ...
                         min_agents_per_timestep=5, prior_alpha=1, prior_beta=1):
    """
    ...
    if growth_rate_uncertainties is not None:
...
def main():
    """
    Main function to demonstrate the two-stage inference process.
    
    This function shows how to:
    1. Calculate p_t series from growth rate data
    2. Use p_t series to infer l_t values
    """
    
    print("=" * 60)
    print("VARIATIONAL INFERENCE FOR ENVIRONMENTAL PARAMETERS")
    print("=" * 60)
    
    # Example: Generate synthetic growth rate data for demonstration
    print("\nGenerating synthetic growth rate data for demonstration...")
    
    np.random.seed(42)
    n_agents = 100
    n_timesteps = 20
    
    # Generate synthetic growth rates (you would replace this with real data)
    growth_rates_data = []
    for agent in range(n_agents):
        # Each agent has a different growth pattern
        agent_growth = np.random.normal(0.02, 0.1, n_timesteps)  # 2% mean growth, 10% volatility
        growth_rates_data.append(agent_growth)
    
    # Generate synthetic population weights
    population_weights = np.random.uniform(0.5, 2.0, n_agents)
    
    print(f"Generated {n_agents} agents with {n_timesteps} timesteps each")
    ...
    try:
        ...
        # Use Bayesian approach for more robust estimation
        p_t_series, p_t_posteriors, timesteps = bayesian_p_estimation(
            growth_rates_data, 
            population_weights,
            growth_rate_uncertainties=None,  # No uncertainties in synthetic data
            min_agents_per_timestep=10,
        ...
        print("=" * 40)
        print(f"p_t series: {len(p_t_series)} timesteps")
    ...
    print("\n" + "=" * 60)
    print("NEXT STEPS:")
    print("1. Replace synthetic data with real growth rate data")
    print("2. Adjust population weights based on your data structure")
    print("3. Fine-tune parameters (delta, n_samples, rolling_window)")
    print("4. Implement the improvements from upgrades.txt")
    print("=" * 60)
